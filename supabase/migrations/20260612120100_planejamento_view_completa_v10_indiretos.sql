-- InfraWork — Planejamento: vw_planejamento_tarefa_completa v10.
--
-- Diferença pra v9: expõe campos de tarefa indireta.
--   * is_indireto:        item.indireto_id IS NOT NULL
--   * indireto_config:    jsonb com toda a config + cache de cálculo
--   * custo_total_calc:   unificado — pra indireta usa cache; pra direta mantém
--                         fórmula custo_unit_snapshot × quantidade_alocada
--   * receita_total_calc: indireta usa cache; direta calcula venda_unitaria × qtd
--   * custo_taxas_calc:   só indireta, do cache (NULL pra direta)
--
-- Conserva todas as colunas v9. Frontend novo lê os campos extras; código
...
select
  ...
  -- Receita unificada: indireta usa cache, direta calcula venda × qtd
  case
