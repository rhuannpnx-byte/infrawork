-- InfraWork — vw_acompanhamento_previsto_x_realizado: EXCLUI itens indiretos.
--
-- Regra: itens de indireto (indireto_id not null — ex.: Administração Central)
-- não têm produção apontada via SIGA (qtd_real sempre 0). Eles não devem
-- figurar em NENHUM controle que compara previsto × realizado via apontamento
-- (tabela Previsto × Realizado, top serviços / aderência do dashboard, alertas
-- de desvio/produção parada). O indireto é tratado SÓ como projeção no Valor
-- Agregado (a partir do baseline, não desta view).
--
-- Antes, o indireto entrava com qtd_real=0 → aparecia "não iniciado/atrasado",
-- poluía o filtro de serviços e podia gerar alertas falsos.
--
-- Fix: filtrar io.indireto_id is null na CTE baseline. A curva-S
-- (vw_acompanhamento_curva_s) já não inclui indireto (sem perfil_semana).
-- Colunas inalteradas → create or replace (sem cascade).
