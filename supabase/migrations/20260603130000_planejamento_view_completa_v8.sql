...
-- vw_planejamento_tarefa_completa v8.
--
-- Mudança em relação à v7 (20260603120100):
--   + expõe progresso físico real-time, derivado do módulo de acompanhamento.
--     Três colunas novas:
--       * progresso_qtd_realizada (numeric) — SUM(producao.qtd × fator_conversao)
--         agregado por item_orcamentario_id da tarefa.
--       * progresso_pct (numeric, 0–1, com cap em 1.0) — qtd_real ÷ qtd_planejada.
--         Usa COALESCE(pt.quantidade_alocada, io.quantidade_referencia) como base.
--         NULL quando não há plano (qtd_plan = 0 / null).
--       * progresso_status (text) — categorização: 'nao_iniciado' | 'em_andamento'
--         | 'concluido'. (Não inclui 'atrasado/em_risco/adiantado' — isso ficou
--         na vw_acompanhamento_previsto_x_realizado, que é por baseline.)
--
-- Importante: a vw_acompanhamento_previsto_x_realizado existente filtra
-- pl.is_baseline=true e retorna 1 linha por tarefa do BASELINE. Aqui queremos
-- progresso pra QUALQUER planejamento (revisão atual, baseline, comparação),
-- então fazemos a agregação inline por item_orcamentario_id, independente de
-- baseline.
--
-- Limitação conhecida: quando há múltiplas tarefas com mesmo item_orcamentario_id
-- no mesmo planejamento (split por trecho), TODAS recebem o mesmo qtd_real
-- agregado. Rateio fino por trecho/posição é trabalho futuro — documentar no PR.
...
select
  ...
  -- qtd_real: SUM(producao.qtd × fator_conversao) agregado por item_orcamentario_id
  coalesce(prog.qtd_realizada, 0)::numeric(14, 4)
                                as progresso_qtd_realizada,
  -- pct: qtd_real / COALESCE(quantidade_alocada, quantidade_referencia)
  -- Cap em 1.0 (>=100% = "concluído", não overflow visual).
  case
    when coalesce(t.quantidade_alocada, i.quantidade_referencia, 0) = 0 then null
    when prog.qtd_realizada is null then 0::numeric(7, 4)
    else
      least(
        1.0::numeric(7, 4),
        round(
          (prog.qtd_realizada::numeric
           / nullif(coalesce(t.quantidade_alocada, i.quantidade_referencia), 0))::numeric,
          4
        )
      )
  end                           as progresso_pct,
  -- status semântico simples (sem 'atrasado/em_risco' — esses dependem
  -- de baseline e ficam na vw_acompanhamento_previsto_x_realizado).
  case
    when coalesce(t.quantidade_alocada, i.quantidade_referencia, 0) = 0 then 'sem_plano'
    when prog.qtd_realizada is null or prog.qtd_realizada = 0 then 'nao_iniciado'
    when prog.qtd_realizada >= coalesce(t.quantidade_alocada, i.quantidade_referencia) then 'concluido'
    else 'em_andamento'
...
-- Agregação on-the-fly de produção realizada por item orçamentário,
-- escopada à obra do planejamento. Sem subquery correlacionada:
-- usa LEFT JOIN LATERAL pra ficar performante mesmo em obras grandes.
left join lateral (
  select sum(pp.qtd * coalesce(sm.fator_conversao, 1))::numeric(14, 4) as qtd_realizada
    from public.acompanhamento_producao pp
    join public.acompanhamento_servico_match sm
      on sm.obra_id = pp.obra_id and sm.siga_servico_executado_id = pp.servico_id
   where sm.obra_id = p.obra_id
     and sm.item_orcamentario_id = t.item_orcamentario_id
     and t.item_orcamentario_id is not null
) prog on true;
