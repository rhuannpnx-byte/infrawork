-- InfraWork — Acompanhamento: planejado vem do CRONOGRAMA, não do ORÇAMENTO.
--
-- Bug: as views usavam io.quantidade_referencia (quantidade ORÇADA do item)
-- como quantidade planejada. Mas o acompanhamento deve comparar o realizado
-- contra o que foi efetivamente ALOCADO no cronograma/baseline, que pode ser
-- menor que o orçamento (nem todo o orçado entra no cronograma).
-- Ex.: fresagem aparecia com 18.000 (orçamento) quando o cronograma aloca
-- apenas 13.750.
--
-- Fix: quantidade planejada passa a ser sum(pt.quantidade_alocada) por item
-- (soma das frentes/tarefas do item no baseline). quantidade_alocada é a qtd
-- do cronograma da tarefa-folha (ver 20260530130100); sua soma por item ≤
-- io.quantidade_referencia.
--
-- 1) vw_acompanhamento_previsto_x_realizado.quantidade_planejada/qtd_plan
-- 2) vw_acompanhamento_curva_s.baseline_meta.qtd_plan (qtd_planejada_total).
--    O planejado_dia da curva-S já vinha do perfil_semana (cronograma); aqui
--    só alinhamos o total de referência à mesma base.
--
-- Colunas de saída inalteradas → create or replace view (sem cascade).
...
with baseline_meta as (
  -- 1 linha por (obra, item). qtd_plan = soma das quantidades alocadas no
  -- cronograma (não a quantidade orçada io.quantidade_referencia).
  select pl.obra_id,
         pt.item_orcamentario_id          as item_id,
         io.codigo,
         io.descricao,
         sum(pt.quantidade_alocada)::numeric(14,4) as qtd_plan
    from public.planejamento pl
    join public.planejamento_tarefa pt on pt.planejamento_id = pl.id
    join public.item_orcamentario   io on io.id = pt.item_orcamentario_id
   where pl.is_baseline = true
     and pt.quantidade_alocada is not null
   group by pl.obra_id, pt.item_orcamentario_id, io.codigo, io.descricao
),
