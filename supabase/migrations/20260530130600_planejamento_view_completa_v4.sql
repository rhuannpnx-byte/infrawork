-- InfraWork — Planejamento (Refator EAP/Marcos/Multi-tarefa, M7):
-- vw_planejamento_tarefa_completa v4.
--
-- Mudanças em relação à v3 (20260529120400):
--   + expõe tipo_no, parent_id, nivel, codigo_eap, nome_custom, quantidade_alocada
--   * JOIN obra_trecho passa de INNER → LEFT (grupos/marcos podem ter trecho NULL)
--   * JOIN item_orcamentario passa de INNER → LEFT (grupos/marcos têm item NULL)
--   * custo_total_tarefa agora pondera por quantidade_alocada (não mais por
--     item.quantidade_referencia) — habilita rateio correto entre N tarefas/item.
--
-- security_invoker = true mantém propagação das policies de planejamento_tarefa.
...
select
  ...
  -- Custo total agora pondera por quantidade_alocada (rateio correto entre N tarefas).
  -- Grupo/marco: custo 0 (rollup é responsabilidade da UI).
  case
