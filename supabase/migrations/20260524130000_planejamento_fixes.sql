-- InfraWork — Planejamento P1.A.fix1
--
-- Fixes encontrados durante simulação end-to-end:
--   1) duracao_dias_uteis_calc precisava de mais precisão. CPU mal calibrada
--      (producao_diaria=1, quantidade grande) estoura numeric(8,2). Sobe pra
--      numeric(12,2) — suporta até ~10 bilhões de dias.
--   2) dias_uteis_bitmask default 62 estava errado (representava ter-sáb).
--      Corrigir para 31 = 0011111 (seg-sex). Atualizar registros existentes.
--   3) View vw_planejamento_tarefa_completa depende da coluna; precisa
--      ser recriada após o ALTER.

-- (1) precisão da duração
drop view if exists public.vw_planejamento_tarefa_completa cascade;

alter table public.planejamento_tarefa
  alter column duracao_dias_uteis_calc type numeric(12,2);

-- (2) default do bitmask + backfill
alter table public.obra_calendario
  alter column dias_uteis_bitmask set default 31;

update public.obra_calendario
   set dias_uteis_bitmask = 31
 where dias_uteis_bitmask = 62;

-- (3) recriar view (usa security_invoker)
